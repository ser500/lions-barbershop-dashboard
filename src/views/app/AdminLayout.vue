...
const locationNav = [
  { label: 'Overview',  icon: 'ph-squares-four', to: '/app/admin/overview' },
  { label: 'Schedule',  icon: 'ph-calendar',     to: '/app/admin/schedule' },
  { label: 'Team',      icon: 'ph-users-three',  to: '/app/admin/team' },
]

const clientRevenueNav = [
  { label: 'Clients',    icon: 'ph-address-book',   to: '/app/admin/clients' },
  { label: 'Revenue',    icon: 'ph-chart-bar',       to: '/app/admin/revenue' },
  { label: 'Membership', icon: 'ph-crown-simple',    to: '/app/admin/membership' },
]

const opsNav = [
  { label: 'Inventory', icon: 'ph-package', to: '/app/admin/inventory' },
  { label: 'AI',        icon: 'ph-robot',   to: '/app/admin/ai', badge: 'AI' },
]
  ...
  <div class="w-full h-screen flex overflow-hidden font-inter" style="background:#050505">
    ...
    <aside class="flex flex-col h-full flex-shrink-0" style="width:240px;background:#0a0a0a;border-right:1px solid #1a1a1a">
      ...
      <nav class="flex-1 px-3 py-4 overflow-y-auto">
        <div class="mb-5">
          <div class="px-3 mb-2 text-xs font-semibold tracking-widest uppercase" style="color:#3f3f46">My Location</div>
          <RouterLink
            v-for="item in locationNav"
            :key="item.to"
            :to="item.to"
        ...
        <div class="mb-5">
          <div class="px-3 mb-2 text-xs font-semibold tracking-widest uppercase" style="color:#3f3f46">Clients & Revenue</div>
          <RouterLink
            v-for="item in clientRevenueNav"
            :key="item.to"
            :to="item.to"
            class="flex items-center gap-3 px-3 py-2.5 rounded-lg mb-1 text-sm transition-all"
            :style="isActive(item.to)
        ...
        <div>
          <div class="px-3 mb-2 text-xs font-semibold tracking-widest uppercase" style="color:#3f3f46">Operations</div>
          <RouterLink
      ...
      <div class="px-4 py-4" style="border-top:1px solid #1a1a1a">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-black flex-shrink-0" style="background:linear-gradient(135deg,#b8960c,#8a7009)">DR</div>
          <div class="flex-1 min-w-0">
            <div class="text-xs font-semibold text-zinc-200 truncate">Devon Richards</div>
            <div class="text-xs text-zinc-600">Location Manager</div>
          </div>
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden" style="background:#0a0a0a">
      <header class="flex items-center justify-between px-6 py-3 flex-shrink-0" style="background:#050505;border-bottom:1px solid #1a1a1a;height:56px">
        <div style="font-size:0.85rem;font-weight:600;color:#e5e7eb">Location Admin</div>
        <div class="flex items-center gap-3">
          <span style="font-size:0.75rem;color:#6b7280">{{ today }}</span>
          <span style="font-size:0.72rem;background:rgba(34,197,94,0.1);color:#22c55e;border:1px solid rgba(34,197,94,0.2);padding:2px 8px;border-radius:20px">Open</span>
        </div>
      </header>
      <LiveFeedTicker />
